...
export const deletePendingUsers = schedule.scheduleJob('1 1 1 1 * *', async function () {
...
export const handleDeletedUsers = schedule.scheduleJob('1 1 1 1 * *', async () => {
    const users = await User.find({
    ...
    for (const user of users) {
        // Example: Send a final notification to the user (if needed)
        // await sendFinalNotification(user.email);

        // Example: Further cleanup actions like removing references in other collections
        // await cleanupUserReferences(user._id);

        // Delete the user after additional logic
...
export const deleteExpiredCoupons = schedule.scheduleJob('1 1 1 1 * *', async function () {
